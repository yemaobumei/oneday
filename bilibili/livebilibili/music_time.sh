...
#		ffmpeg -re  -i upload.flv -c copy -f flv "rtmp://xl.live-send.acg.tv/live-xl/?streamname=live_25303175_5745325&key=71771fb7bb37ccbdc17888f178f56075"
	...
	ffmpeg -loop 1 -re -i "0${rnd}.jpg"  -i "${file}"  -vf "drawtext=text='点歌：点歌+歌名 切歌：切歌 切歌可能需要手动刷新直播间页面 ':fontfile=${font_path}:fontsize=24:fontcolor=yellow@0.8:x=w-tw-30:y=th,drawtext=text='正在播放：${file}   当前无人点歌':fontfile=${font_path}:fontsize=24:fontcolor=yellow@0.8:x=w-tw-30:y=2*th+10"  -strict experimental -c:v libx264 -c:a aac -b:a 192k   -shortest  -f flv "rtmp://xl.live-send.acg.tv/live-xl/?streamname=live_25303175_5745325&key=71771fb7bb37ccbdc17888f178f56075"
